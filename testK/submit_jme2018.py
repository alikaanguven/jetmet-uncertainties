...
def process_signal(sig_json, out_dir, tier, sig_samples=None):
        ...
        for root, dirs, files in os.walk(sig_data[tier]["dir"][sample]):
            for file in files:
                if fnmatch.fnmatch(file, "*.root"):
                    ...
                    command = f'submit_to_cpu.sh "python jetmet2018mc_post.py {sample_out_path} {nanoAOD_path}"'
    ...
    job_ids_file = os.path.join(out_dir, 'job_ids_2018.json')
